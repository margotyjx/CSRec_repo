...
# Only needed for fearec and duorec
    ...
    def __getitem__(self, index):
        if self.data_type in ['generation']:
            pass
        else:
        ...
        elif self.data_type in ['generation']:
            input_ids = self.user_seq[index]
...
def get_dataloder(args,seq_dic):
    ...
    train_dataset = RecDataset_obs(args, seq_dic['user_seq'], data_type='train')
    ...
    eval_dataset = RecDataset_obs(args, seq_dic['user_seq'], data_type='valid')
    ...
    test_dataset = RecDataset_obs(args, seq_dic['user_seq'], data_type='test')
...
def get_gen_dataloder(args, seq_dic):
    gen_dataset = RecDataset_obs(args, seq_dic['user_seq'], data_type='generation')
    gen_sampler = SequentialSampler(gen_dataset)
    gen_dataloader = DataLoader(gen_dataset, sampler=gen_sampler, batch_size=args.batch_size, num_workers=args.num_workers)
