...
class BSARecModel(SequentialRecModel):
    ...
    def forward(self, input_ids, user_ids=None, all_sequence_output=False):
        ...
        #        item_embeddings = self.item_embeddings(sequence) position_embeddings = self.position_embeddings(position_ids) 
        # add together
    ...
    def calculate_loss_treatment(self, input_ids, answers, neg_answers, same_target, user_ids):
        sigmoid = torch.nn.Sigmoid()

        seq_output = self.forward(input_ids)
        seq_output = seq_output[:, -1, :]
        item_emb = self.item_embeddings.weight
        prob_logits = sigmoid(torch.matmul(seq_output, item_emb.transpose(0, 1)))

        ind_range = (torch.arange(0,len(answers))).to(answers.device)
        inds = torch.concatenate((ind_range.reshape(-1,1),answers.reshape(-1,1)), dim=1)
        prob_rec = prob_logits[inds[:,0],inds[:,1]]

        # print(prob_rec.type())
        # print((torch.ones_like(answers).to(answers.device)))
